...
document.getElementById('sidebar-slot').textContent  = '⏰ ' + timeSlot + ' Match';
...
document.title = 'Register for ' + eventName + ' — Sports Fest 2026';
...
function validateForm(data) {
  ...
  ['name', 'email', 'mobile', 'registerNumber', 'department', 'role'].forEach(clearError);
  ...
  try {
    ...
    if (response.ok) {
      ...
      document.getElementById('success-message').textContent =
        `You've registered for ${formData.eventName} (${formData.timeSlot} Match). Get ready to play!`;
